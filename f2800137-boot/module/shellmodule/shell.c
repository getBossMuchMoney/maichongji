...
Uint16 shellGets(Uint16 *buf,Uint32 len)
{
    Uint16 i=0;
    int c;
...
//    Uint16 len = 0;
    ...
    case eShell_RX: //rx
		...
		{
			shellLine[count++] = c;
			shellLinebuf[shelllinecnt] = c;
			shelllinecnt = (shelllinecnt+1)&0xff;
			if(count >= SHELL_LINE_MAX_LEN)
			{
				count = 0;
			}
		}
    ...
    case eShell_RX_WAIT: //wait rx finish
		...
		else if(getTimerTickCur() - curtime > 5)
        {
            step = eShell_JUDGE;
        }
        break;
    case eShell_JUDGE: //judge
        if((count >= 2) && (shellLine[count-2] == '\r') && (shellLine[count-1] == '\n'))
        {
